...
void Pipeline::fit(const ArrayXXd &X, const ArrayXd &y) {
  ...
  for (int i = 0; i < steps.size() - 1; i++)
    dynamic_cast<TransformerMixin &>(*steps[i].second)
        .fit_transform_inplace(Xcopy);
...
void Pipeline::inverse_transform_inplace(ArrayXXd &X) {
  ...
  for (auto step = steps.rbegin(); step != steps.rend(); ++step)
    dynamic_cast<TransformerMixin &>(*(*step).second)
...
BaseEstimator *Pipeline::operator[](int idx) { return steps[idx].second; }
Pipeline Pipeline::operator()(int start, int stop) {
  // check to make sure things are in range
  if (start < 0)
    throw invalid_argument("Lower index is too low");
  if (stop > steps.size())
    throw invalid_argument("Upper index is too high");
  // Make copy of itself
